...
//	},
	...
	next: function (element, selector) {
		...
		} while (node && !js.dom.Relation.test(selector));
		return node;
	},
	...
	nextAll: function (element, selector) {
		var ret = [];
		for (var node = element.nextSibling; node && js.dom.Relation.test(selector); node = node.nextSibling) {
			ret.push(node);
		}
		return ret;
	},
	...
	prevAll: function (element, selector) {
		var ret = [];
		for (var node = element.previousSibling; node && js.dom.Relation.test(selector); node = node.previousSibling) {
			ret.push(node);
		}
		return ret;
	},
	...
	firstChild: function (element, selector) {
		for (var node = element.firstChild; node && !js.dom.Relation.test(selector);node = node.nextSibling);
		return node;
	},
	...
	lastChild: function (element, selector) {
		for (var node = element.lastChild; node && !js.dom.Relation.test(selector);node = node.previousSibling);
		return node;
	},
